export default async function handler(req, res) {
    ...
    if (action === 'search') {
        ...
        target_url =
            `${BASE_URL}/discover/movie?api_key=${API_KEY}&with_original_language=te|ta|ml|kn&sort_by=popularity.desc&page=${page}`;
        ...
        if (type === 'trending') {
            ...
            target_url =
                `${BASE_URL}/trending/${mode}/week?api_key=${API_KEY}&page=${page}`;

        } else {

            target_url =
                `${BASE_URL}/discover/${mode}?api_key=${API_KEY}&sort_by=popularity.desc&page=${page}`;
        }
    }
